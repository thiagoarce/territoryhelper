...
| `tp_agendamentos` / `tp_agendamento_excecoes` / `tp_agendamento_participantes` | Agendamento = equipamento + ponto (fixo ou avulso) + data/hora + recorrência (nenhuma/diária/semanal/quinzenal/mensal); exceções tratam "só esta ocorrência"; participantes SEM capacidade fixa (`origem` inscrição/designação) — TP-F, `/admin/tp/*` + inscrição em `/publicador/arranjo`. `tp_agendamentos.origem` (admin/reserva, A22-f3): publicador **aprovado** (`profiles.tp_aprovado`) pode criar sua própria reserva pontual numa célula vazia da grade do mês publicado (`/publicador/tp`, RLS dedicada), convidando outros aprovados; só quem criou (ou admin) cancela. **Montagem automática** (A22-f4): `$lib/tp-montagem.ts` — heurística PURA (sem I/O, testada em `tests/tp-montagem.test.ts`) que sugere designações a partir de disponibilidade do mês (`tp_disponibilidade_mes`) + turnos-alvo (ocorrências expandidas de `tp_agendamentos`) + `tp_preferencias.transporta_carrinho`, respeitando aprovação e nunca sobrepondo horários; painel em `/admin/tp` (dentro do ciclo mensal) gera a proposta no CLIENTE, admin revisa/desmarca e publica (grava em `tp_agendamento_participantes`, sem duplicar notificação — quem notifica é a transição de fase pra "publicado") |
