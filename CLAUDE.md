...
| `quadras_conclusoes` | histórico append-only de conclusões (data, autor, `marcado_em` timestamptz). TODA escrita de conclusão precisa passar por `$lib/server/conclusao.ts` (`registrarConclusaoQuadra`/`desfazerConclusaoQuadra`) — nunca fazer `update quadras set data_conclusao=...` direto fora dali; já rolou bug real de 2 actions (concluir em campo) atualizando só `quadras.data_conclusao` e pulando o histórico, invisível pro S-13/dashboard/campanha (migration 084 fez o backfill). `marcado_em` guarda a hora que o SERVO INFORMOU que o trabalho foi feito (não a hora do registro) — telas de concluir (`/publicador/quadra/[id]`, Casa a casa "seu grupo", `/admin` Geral) têm campo de hora além da data, pré-preenchido com `hora_inicio` do arranjo vinculado quando existe (`carregarQuadraCampo::arranjoHoraInicio`); `$lib/utils/data.ts::horaBrasilParaIso` converte data+hora local pra timestamptz. Sem hora informada (ex: modo histórico do admin), cai no `default now()` da coluna — pior proxy, mas não quebra nada. Migration 087 fez backfill dos registros antigos com uma estimativa por dia da semana (não é hora real, só destrava a análise manhã/tarde/noite do dashboard com *algum* dado histórico). **Migration 090**: RLS de `quadras` só permitia escrita de admin, então o UPDATE de `data_conclusao` do dirigente era descartado em silêncio (0 linhas, sem erro) — dirigente ganhou policy de UPDATE + trigger `quadras_guard_nao_admin` (não-admin só mexe em `data_conclusao`) e DELETE em `quadras_conclusoes` (pro "Desfazer" funcionar) |
...
- `UPDATE`/`DELETE` barrado por RLS **não devolve erro**: a policy filtra
  ...
  assim que "marcar concluída" do dirigente passou meses dando toast
  verde sem mudar nada — `quadras` só tinha policy de escrita pra admin
  (corrigido na migration 090: policy de UPDATE pro dirigente + trigger
  `quadras_guard_nao_admin` limitando não-admin à coluna
  `data_conclusao`). Escrita que o usuário PRECISA ver acontecer usa
  `{ count: 'exact' }` e trata `count === 0` como erro (padrão em
  `$lib/server/conclusao.ts`).
