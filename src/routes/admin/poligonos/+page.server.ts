...
import {
  criarPontoReferencia,
  atualizarPontoReferencia,
  excluirPontoReferencia,
} from "$lib/server/pontos";
...
export const actions: Actions = {
  ...
  autoVincular: async ({ locals }) => {
    ...
    const { data, error } = await locals.supabase.rpc(
      "auto_vincular_enderecos" as any,
    );
    if (error) return fail(400, { erro: error.message });
    const r = (data as any)?.[0];
    return {
      ok: true,
      msg: `${r?.vinculados ?? 0} endereço(s) vinculado(s) automaticamente (${r?.sem_match ?? 0} sem polígono correspondente).`,
    };
  },
  ...
  vincularManual: async ({ request, locals }) => {
    ...
    if (localIds.length === 0 || !quadraId)
      return fail(400, { erro: "local_ids e quadra_id obrigatórios" });
    ...
    return {
      ok: true,
      msg: `${localIds.length} endereço(s) vinculado(s) a ${quadraId}`,
    };
  },
  ...
  toggleAtivacao: async ({ request, locals }) => {
    ...
    const { error } = await locals.supabase
      .from("locais")
      .update({ nao_visitar: !ativar })
      .in("id", localIds);
    if (error) return fail(400, { erro: error.message });
    return {
      ok: true,
      msg: `${localIds.length} endereço(s) ${ativar ? "ativado(s)" : "desativado(s)"}`,
    };
  },
  ...
  criarTerritorio: async ({ request, locals }) => {
    ...
    if (quadrasIds.length > 0) {
      const { error: errQ } = await locals.supabase
        .from("quadras")
        .update({ territorio_id: id })
        .in("id", quadrasIds);
      if (errQ)
        return fail(400, {
          erro:
            "Território criado mas falhou ao vincular quadras: " + errQ.message,
        });
    }
    return {
      ok: true,
      msg: `Território "${nome}" criado com ${quadrasIds.length} quadra(s)`,
    };
  },
  ...
  adicionarQuadrasAoTerritorio: async ({ request, locals }) => {
    ...
    const quadrasIds = fd
      ...
      .map((v) => String(v))
      .filter(Boolean);
    if (!id || quadrasIds.length === 0)
      return fail(400, { erro: "território + quadras obrigatórios" });
    const { error } = await locals.supabase
      .from("quadras")
      .update({ territorio_id: id })
      .in("id", quadrasIds);
  ...
  criarTce: async ({ request, locals }) => {
    ...
    if (localIds.length === 0)
      return fail(400, { erro: "Selecione endereços comerciais" });
    const { data, error } = await locals.supabase.rpc(
      "criar_tce" as any,
      {
        p_nome: nome,
    ...
    return {
      ok: true,
      msg: `TCE "${nome}" criado (${localIds.length} endereço(s))`,
      id: data,
    };
  },
  ...
  salvarPoligonoQuadra: async ({ request, locals }) => {
    ...
    try {
      ...
      return fail(400, { erro: "GeoJSON inválido" });
    }
    const { error } = await locals.supabase.rpc(
      "salvar_quadra_poligono" as any,
      {
        p_id: id,
        p_geojson: geojson,
        p_color: color,
        p_territorio_id: territorioId,
        p_criar: criar,
      } as any,
    );
    if (error) return fail(400, { erro: error.message });
    return {
      ok: true,
      msg: criar ? `Quadra ${id} criada` : `Forma de ${id} salva`,
    };
  },
  ...
  juntarQuadras: async ({ request, locals }) => {
    ...
    const ids = fd
      ...
      .filter(Boolean);
    if (ids.length < 2)
      return fail(400, { erro: "Selecione ao menos 2 quadras" });
    const { data, error } = await locals.supabase.rpc(
      "quadras_join" as any,
      { p_ids: ids } as any,
    );
  ...
  dividirQuadra: async ({ request, locals }) => {
    ...
    try {
      ...
      return fail(400, { erro: "Linha inválida" });
    }
    const { error } = await locals.supabase.rpc(
      "dividir_quadra" as any,
      {
        p_id: id,
        p_line: line,
        p_novo_id: novoId,
  ...
  excluirQuadra: async ({ request, locals }) => {
    ...
    return {
      ok: true,
      msg: `Quadra ${id} excluída (endereços ficaram sem quadra)`,
    };
  },
  ...
  unificarClusterQuadra: async ({ request, locals }) => {
    ...
    if (!locaisDaQuadra || locaisDaQuadra.length === 0)
      return fail(400, { erro: "Quadra sem endereços" });

    const contagem = new Map<
      string,
      { setor: string | null; quadra_ibge: string | null; qtd: number }
    >();
    ...
    const { error: errU } = await locals.supabase
      .from("locais")
      .update({
        setor: majoritario.setor,
        quadra_ibge: majoritario.quadra_ibge,
      })
      .eq("quadra_id", quadraId);
    if (errU) return fail(400, { erro: errU.message });
    return {
      ok: true,
      msg: `Quadra ${quadraId} unificada (${locaisDaQuadra.length} endereço(s) normalizado(s))`,
    };
  },
  ...
  renomearQuadra: async ({ request, locals }) => {
    ...
    const { data: existe } = await locals.supabase
      .from("quadras")
      .select("id")
      .eq("id", idNovo)
      .maybeSingle();
    ...
    const { data: antiga } = await locals.supabase
      .from("quadras")
      .select("*")
      .eq("id", idAntigo)
      .maybeSingle();
    ...
    const { error: e1 } = await locals.supabase
      .from("quadras")
      .insert({ ...antiga, id: idNovo });
    if (e1) return fail(400, { erro: "Erro criando nova: " + e1.message });

    // 2. Atualiza refs
    await locals.supabase
      .from("locais")
      .update({ quadra_id: idNovo })
      .eq("quadra_id", idAntigo);
    await locals.supabase
  ...
  reverterCuradoria: async ({ request, locals }) => {
    ...
    const { data: linha, error: errBusca } = await locals.supabase
      ...
      .single();
    if (errBusca || !linha)
      return fail(404, { erro: "Registro de curadoria não encontrado" });

    if (linha.tipo === "criacao") {
      if (!linha.local_id)
        return fail(400, { erro: "Sem local_id pra reverter criação" });
    ...
    if (linha.tipo === "exclusao") {
      ...
      if (!snapshot)
        return fail(400, { erro: "Sem cópia de segurança para restaurar" });
      if (linha.entidade === "local") {
        const local = snapshot.local as Record<string, unknown> | undefined;
        const unidades = Array.isArray(snapshot.unidades)
          ? snapshot.unidades
          : [];
        if (!local) return fail(400, { erro: "Cópia do endereço incompleta" });
        const { error: errLocal } = await locals.supabase
          .from("locais")
          .insert(local);
        if (errLocal) return fail(400, { erro: errLocal.message });
        if (unidades.length > 0) {
          const { error: errUnidades } = await locals.supabase
            .from("unidades")
            .insert(unidades);
          if (errUnidades)
            return fail(400, {
              erro:
                "Endereço restaurado, mas faltaram unidades: " +
                errUnidades.message,
            });
        }
      } else {
        const { error: errUnidade } = await locals.supabase
          .from("unidades")
          .insert(snapshot);
