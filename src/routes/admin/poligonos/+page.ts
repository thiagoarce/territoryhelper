...
export const load: PageLoad = async () => {
  ...
  const { data: tceRows } = await supabase
    .from('tces_geo')
    .select('id, nome, tipo, status, prazo, publicador_id, poly_geojson')
    .order('criado_em', { ascending: false });
  const publicadores = await listarPublicadores(supabase);
  ...
  const { data: curadoriaRows } = await supabase
    .from('curadoria_edicoes')
    .select('id, local_id, unidade_id, publicador_id, tipo, antes, depois, criado_em')
    .eq('status', 'pendente')
    .order('criado_em', { ascending: false });
  const localIdsCuradoria = Array.from(new Set((curadoriaRows ?? []).map((c: any) => c.local_id).filter((v: any): v is number => v != null)));
