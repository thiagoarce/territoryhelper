<script lang="ts">
  ...
  import { ocorrenciasDaSemana, agruparPorDia, semanaAtual, DIAS_SEMANA, DIAS_ORDENADOS, type ArranjoBase } from '$lib/arranjos';
  ...
  function togglePub(id: string) {
    if (pubsSel.has(id)) pubsSel.delete(id);
    else pubsSel.add(id);
...
<div class="p-4 space-y-3">
  <div>
    ...
    <p class="text-sm text-slate-500">Saídas em grupo dessa semana — coordenadas pelo admin.</p>
  ...
  {#if ocorrencias.length === 0}
    <Card padding="md">
      <div class="text-center py-8">
        ...
        <div class="text-sm text-slate-500">O admin ainda não marcou saídas pra esta semana.</div>
      </div>
    </Card>
  {:else}
            ...
            <div class="grid gap-2">
              {#each ocPorDia[dia] ?? [] as oc (oc.arranjo.id + '-' + oc.data)}
                ...
                <Card padding="md">
                  <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1 min-w-0">
                      <div class="flex items-center gap-2 flex-wrap">
                        ...
                        {#if a.recorrente}<span class="text-[10px] bg-blue-100 text-blue-700 px-1.5 rounded">semanal</span>{/if}
                      </div>
                      <div class="text-sm text-slate-600 mt-0.5 flex flex-wrap gap-x-3 gap-y-0.5">
                        ...
                        {#if a.dirigente_id}<span>👤 {data.dirigentes[a.dirigente_id] ?? '?'}{ehMeu ? ' (você)' : ''}</span>{/if}
                      </div>
                  ...
                  {#if data.podeCoordenar && ehMeu && m?.tipo_territorio === 'quadras' && (a.quadras_ids?.length ?? 0) > 0}
  ...
  {#if arranjoDist}
    ...
    >
      ...
      <div class="text-sm">
        ...
        <p class="text-xs text-slate-500 mt-2">Cada publicador selecionado vai receber uma designação pessoal com todas essas quadras.</p>
      </div>
